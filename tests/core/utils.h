                 ...
                 const Avogadro::Core::Molecule& m2)
{
  ...
  EXPECT_EQ(m1.atomicNumbers(), m2.atomicNumbers());
  EXPECT_EQ(m1.atomPositions2d(), m2.atomPositions2d());
  EXPECT_EQ(m1.atomPositions3d(), m2.atomPositions3d());
  EXPECT_EQ(m1.data("test").toString(), m2.data("test").toString());
  EXPECT_EQ(m1.bondPairs(), m2.bondPairs());
  EXPECT_EQ(m1.bondOrders(), m2.bondOrders());
  ...
  for (size_t i = 0; i < m1.meshCount(); i++) {
    ...
    EXPECT_EQ(mesh1->vertices(), mesh2->vertices());
    EXPECT_EQ(mesh1->normals(), mesh2->vertices());
