...
TEST(CjsonTest, crystal)
{
  ...
  EXPECT_LT(std::fabs((float)unitCell->a() - 2.95812f), 1e-5f);
  EXPECT_LT(std::fabs((float)unitCell->b() - 4.59373f), 1e-5f);
  EXPECT_LT(std::fabs((float)unitCell->c() - 4.59373f), 1e-5f);
  EXPECT_LT(std::fabs((float)unitCell->alpha() - (.5f * PI_F)), 1e-5f);
  EXPECT_LT(std::fabs((float)unitCell->beta() - (.5f * PI_F)), 1e-5f);
  EXPECT_LT(std::fabs((float)unitCell->gamma() - (.5f * PI_F)), 1e-5f);

  Atom atom = molecule.atom(5);
  EXPECT_EQ(atom.atomicNumber(), 8);
  EXPECT_LT(std::fabs((float)atom.position3d().x() - 1.479060f), 1e-5f);
  EXPECT_LT(std::fabs((float)atom.position3d().y() - 3.699331f), 1e-5f);
  EXPECT_LT(std::fabs((float)atom.position3d().z() - 0.894399f), 1e-5f);
